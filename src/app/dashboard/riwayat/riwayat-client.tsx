...
import type { LeaveHistoryItem } from "./page";

interface RiwayatClientProps {
  allLeaveHistory: LeaveHistoryItem[];
}
...
export function RiwayatClient({ allLeaveHistory }: RiwayatClientProps) {
                 ...
                 <Select value={String(selectedYear)} onValueChange={(value) => setSelectedYear(Number(value))}>
                    ...
                    <SelectContent>
                        {(availableYears.length > 0 ? availableYears : [currentYear]).map(year => (
                         ...
                         {availableYears.length === 0 && <SelectItem value={String(currentYear)}>Tahun {currentYear}</SelectItem>}
                    </SelectContent>
                </Select>
            </div>
        </div>
